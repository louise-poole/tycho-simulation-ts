...
impl SimulationClient {
    async fn new(
        ...
        // Get initial state
        {
            ...
            if let Some(Ok(update)) = stream.next().await {
                ...
                log(&format!(
                    "Received update with {} new pairs",
    ...
    async fn find_pool_for_pair(
        ...
        token0: &Token,
        token1: &Token,
    ) -> Option<(Box<dyn ProtocolSim>, ProtocolComponent)> {
        log(&format!(
            "Looking for pool with tokens {} and {}",
        ...
        let result = states
            ...
            .find(|(_, comp)| {
                ...
                (pool_tokens[0].address == token0.address
                    && pool_tokens[1].address == token1.address)
                    || (pool_tokens[0].address == token1.address
                        && pool_tokens[1].address == token0.address)
            })
        ...
        rt.block_on(async {
            ...
            .await
            {
                ...
                Err(e) => {
                    ...
                    deferred.settle_with(&channel, move |mut cx| cx.error(error_msg));
                }
            }
        });
    });
...
fn get_protocol_name(state: &dyn ProtocolSim) -> String {
            ...
            if let Some((state, _)) = client_ref.find_pool_for_pair(&token0, &token1).await {
                match state.spot_price(&token0, &token1) {
                    Ok(price) => {
                        ...
                        deferred.settle_with(&channel, move |mut cx| Ok(cx.number(price)));
                    }
                    Err(e) => {
                ...
                deferred.settle_with(&channel, move |mut cx| {
                    cx.error("No pool found for token pair")
                });
            }
        });
    });
    ...
    std::thread::spawn(move || {
        rt.block_on(async {
                ...
                if is_matching_pair {
                    ...
                    for &amount_in in &amounts {
                        let amount_in_scaled =
                            (amount_in * 10f64.powi(token0.decimals as i32)) as u128;
                        match state.get_amount_out(amount_in_scaled.into(), &token0, &token1) {
                            Ok(result) => {
                                let amount_out = result.amount.to_f64().unwrap()
                                    / 10f64.powi(token1.decimals as i32);
                            ...
                            Err(e) => {
                                log(&format!(
                                    "Error calculating amount out for pool {}: {}",
                                    pool_id, e
                                ));
                                has_error = true;
                                break;
            ...
            deferred.settle_with(&channel, move |mut cx| {
                ...
                for (i, (pool_id, amounts_out, gas_estimates, protocol)) in
