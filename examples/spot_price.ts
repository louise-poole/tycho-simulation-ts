...
async function runBenchmark(): Promise<void> {
    try {
        ...
        console.log(
            `Average time per amount: ${(duration / amountsIn.length).toFixed(2)}ms`
        );
        console.log(
            `Amounts processed per second: ${(
                amountsIn.length /
                (duration / 1000)
            ).toFixed(2)}`
        );
        ...
        if (results && results.length > 0) {
            ...
            for (let i = 0; i < 5; i++) {
                console.log(
                    `  ${amountsIn[i]} WETH -> ${firstPool.amountsOut[i]} USDT (gas: ${firstPool.gasEstimates[i]})`
                );
            }
        }
    } catch (error) {
